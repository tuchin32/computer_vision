...
def planarAR(REF_IMAGE_PATH, VIDEO_PATH):
    ...
    while (video.isOpened()):
        ...
        if ret:  ## check whethere the frame is legal, i.e., there still exists a frame
            ...
            aruco_corners = aruco.detectMarkers(frame, arucoDict, parameters=arucoParameters)

            # only use the first (largest) corner
            corners = np.array(aruco_corners[0][0][0], dtype=np.float32)

            if corners.ndim == 2 and corners.shape[0] == ref_corns.shape[0]:
