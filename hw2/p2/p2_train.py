...
def plot_learning_curve(logfile_dir, result_lists):
    ...
    #     }
    x = range(1, len(result_lists['train_acc']) + 1)
    for key, value in result_lists.items():
        plt.plot(x, value)
        plt.title(key.replace('_', ' '))
        plt.xlabel('epoch')
        plt.ylabel((key.split('_'))[1])
        plt.savefig(os.path.join(logfile_dir, key + '.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=1, gamma=0.911)
